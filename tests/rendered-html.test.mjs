...
test("Elektrik alanı dirençlerin seri ve paralel bağlanması deneyini sunar", async () => {
  ...
  assert.match(page, /İki direnç, üç anahtar, tek devre/);
  assert.match(page, /SADE DENEY DÜZENEĞİ/);
  assert.match(page, /A: 100, B: 150/);
  assert.match(page, /switches: \[1\]/);
  assert.match(page, /switches: \[2, 3\]/);
  assert.match(page, /Seri köprü/);
  assert.match(page, /A kolu/);
  assert.match(page, /B kolu/);
  assert.match(page, /SimpleCircuit/);
  assert.match(page, /simple-resistor-circuit/);
  assert.match(page, /\[1, 2, 3\]\.map/);
  assert.match(page, /BenchResistor label="A"/);
  assert.match(page, /BenchResistor label="B"/);
  assert.match(page, /resistor-switch-/);
  assert.match(page, /analyzeCircuit/);
  assert.match(page, /Seri bağlantı: A — B/);
  assert.match(page, /Paralel bağlantı: A ∥ B/);
  assert.match(page, /number === 1/);
  assert.match(page, /filter\(\(item\) => item !== 1\)/);
  assert.doesNotMatch(page, /SABİT DİRENÇ BAĞLANTI PANOSU|Array\.from\(\{ length: 11 \}|label="C"|label="D"/);
  assert.match(page, /Ölçüm tablosu/);
  assert.match(page, /TYMM · KISA DENEY RAPORU/);
  assert.doesNotMatch(page, /Kirchhoff|Math\.random|NOISE|hata|belirsiz/i);
});

test("direnç bağlantıları düzeneği sade devre ve ölçüm alanları içerir", async () => {
  const css = await readFile(new URL("app/globals.css", projectRoot), "utf8");
  ...
  assert.match(css, /\.rc4-circuit\s*\{/);
  assert.match(css, /\.rc4-circuit-field\s*\{/);
  assert.match(css, /\.rc4-wire\s*\{/);
  assert.match(css, /\.rc4-switch\s*\{/);
  assert.match(css, /\.rc4-switch-1\s*\{/);
  assert.match(css, /\.rc4-switch-3\s*\{/);
  assert.match(css, /\.rc4-resistor\s*\{/);
  assert.match(css, /\.rc4-resistor-a\s*\{/);
  assert.match(css, /\.rc4-resistor-b\s*\{/);
  assert.match(css, /@keyframes rc4-current-dot/);
  assert.match(css, /@keyframes rc4-resistor-glow/);
