...
test("Elektrik alanı dirençlerin seri ve paralel bağlanması deneyini sunar", async () => {
  ...
  assert.match(page, /Bağlantıyı gör, devreyi tamamla/);
  assert.match(page, /ANAHTARLI DENEY PANOSU/);
  ...
  assert.match(page, /Kaynağın \(\+\) ucunu A direncinin üst ucuna bağlar/);
  assert.match(page, /A direncinin alt ucunu kaynağın \(−\) ucuna bağlar/);
  assert.match(page, /Her şalter bağladığı iki noktayı yazar/);
  assert.match(page, /Sıradaki anahtarı göster/);
  assert.match(page, /CircuitRoute/);
...
test("direnç bağlantıları düzeneği gerçekçi pano ve ölçüm alanları içerir", async () => {
  ...
  assert.match(css, /\.rc2-main-board\s*\{/);
  assert.match(css, /\.rc2-circuit-window\s*\{/);
  assert.match(css, /\.rc2-inline-switch\s*\{/);
  assert.match(css, /\.rc2-switch-mechanism\s*\{/);
  assert.match(css, /\.rc2-resistor-body\s*\{/);
  assert.match(css, /\.rc2-parallel-route,/);
  assert.match(css, /@keyframes rc2-current-dot/);
