...
test("Elektrik alanı dirençlerin seri ve paralel bağlanması deneyini sunar", async () => {
  ...
  assert.match(page, /Tek pano, değişen bağlantılar/);
  assert.match(page, /SABİT DİRENÇ BAĞLANTI PANOSU/);
  assert.match(page, /A: 100, B: 150, C: 220, D: 330/);
  assert.match(page, /switches: \[1, 3, 7, 10\]/);
  assert.match(page, /switches: \[1, 2, 6, 4, 10\]/);
  assert.match(page, /switches: \[1, 3, 5, 7, 9\]/);
  assert.match(page, /Pozitif ana hattı A direncinin üst ucuna bağlar/);
  assert.match(page, /A direncinin alt ucunu negatif ana hatta bağlar/);
  assert.match(page, /FixedCircuitBoard/);
  assert.match(page, /fixed-resistor-network/);
  assert.match(page, /Array\.from\(\{ length: 11 \}/);
  assert.match(page, /BoardResistor label="A"/);
  assert.match(page, /BoardResistor label="B"/);
  assert.match(page, /BoardResistor label="C"/);
  assert.match(page, /BoardResistor label="D"/);
  assert.match(page, /resistor-switch-/);
  assert.match(page, /1: \["P", "AT"\]/);
  assert.match(page, /3: \["AB", "BB"\]/);
  assert.match(page, /4: \["BT", "CT"\]/);
  assert.match(page, /10: \["X", "N"\]/);
  assert.match(page, /analyzeCircuit/);
  assert.match(page, /solveLinearSystem/);
  assert.match(page, /Paralel bağlantı:/);
  assert.match(page, /Seri bağlantı:/);
  assert.match(page, /Birleşik bağlantı:/);
  assert.doesNotMatch(page, /function CircuitRoute|if \(task\.kind ===/);
  assert.match(page, /A — \(B ∥ C\) — D/);
...
test("direnç bağlantıları düzeneği gerçekçi pano ve ölçüm alanları içerir", async () => {
  ...
  assert.match(css, /\.rc2-lab\s*\{/);
  assert.match(css, /\.rc2-workspace\s*\{/);
  assert.match(css, /\.rc2-workbench\s*\{/);
  assert.match(css, /\.rc3-fixed-board\s*\{/);
  assert.match(css, /\.rc3-network\s*\{/);
  assert.match(css, /\.rc3-main-rail,/);
  assert.match(css, /\.rc3-wire\.wire-a\s*\{/);
  assert.match(css, /\.rc3-wire\.wire-common-return\s*\{/);
  assert.match(css, /\.rc3-board-switch\s*\{/);
  assert.match(css, /\.rc3-switch-1\s*\{/);
  assert.match(css, /\.rc3-switch-11\s*\{/);
  assert.match(css, /\.rc3-board-resistor\s*\{/);
  assert.match(css, /\.rc3-resistor-a\s*\{/);
  assert.match(css, /\.rc3-resistor-d\s*\{/);
  assert.match(css, /@keyframes rc3-resistor-flow/);
  assert.match(css, /\.rc2-power-supply,/);
  assert.match(css, /\.rc2-digital-meter\s*\{/);
  assert.match(css, /\.rc2-controls\s*\{/);
  assert.match(css, /\.rc2-data-grid\s*\{/);
  assert.match(css, /\.rc2-report\s*\{/);
});
